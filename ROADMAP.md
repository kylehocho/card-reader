...
- Continue splitting `WalletPrototype.tsx` into smaller behavior-owned pieces. The add-card presentation state was extracted on 2026-07-17, add-card sheet rendering moved into `AddCardSheet.tsx` on 2026-07-18, auth/profile sheet composition moved into `ProfileAccessBoundary.tsx` on 2026-07-19, fixture-backed onboarding UI evidence was captured on 2026-07-20, selected-card mutation outcomes moved into the wallet navigation boundary on 2026-07-21, selection-outcome evidence was added on 2026-07-22, browser-rendered signed-in fixture contract smoke coverage was added on 2026-07-23, live signed-in manual-card persistence smoke tooling was added on 2026-07-24, Plaid card-match persistence moved behind an authenticated app API on 2026-07-25, signed-in Plaid exchange/card-match smoke tooling was added on 2026-07-26, a shared signed-in smoke dependency preflight was added on 2026-07-28, anonymous demo-card projection moved into `useDemoWalletCards.ts` on 2026-07-30, protected wallet access gates moved into `useWalletAccessGates.ts` on 2026-07-31, Plaid wallet-card projection/local fallback storage moved into `usePlaidWalletCards.ts` on 2026-08-01, anonymous demo-card append/selection results moved into `useDemoWalletCards.ts` on 2026-08-02, signed-in wallet-analysis loading moved into `useWalletAnalysis.ts` on 2026-08-03, wallet-analysis view projection moved into `useWalletAnalysisViews.ts` on 2026-08-04, Plaid wallet-card merge/selection policy moved into `usePlaidWalletCards.ts` on 2026-08-05, profile notification settings rendering/state moved into `NotificationSettingsScreen.tsx` plus `useNotificationSettings.ts` on 2026-08-06, connected-account removal confirmation rendering moved into `ConnectedAccountRemovalSheet.tsx` on 2026-08-07, and the wallet card stack renderer moved into `WalletStack.tsx` on 2026-08-08. Next candidate is refreshing the local Supabase service-role credential, rerunning the preflight plus both signed-in smoke commands, then adding browser-driven signed-in Plaid onboarding evidence against production.
