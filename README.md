...
The app now uses Supabase Auth and the `profiles` table whenever `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` are configured. `NEXT_PUBLIC_SUPABASE_ANON_KEY` is still accepted as a fallback for older environments. Without those env vars, it falls back to the local prototype auth flow so UI work remains easy.
