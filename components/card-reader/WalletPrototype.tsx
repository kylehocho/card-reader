      ...
      <div className="min-h-screen bg-[#080b13] text-white">
        ...
        {showScanner && (
          <AddCardSheet
            ...
            connectPlaidSandbox={() => void connectPlaidSandbox(loadPlaidLinkScript)}
            draftCard={draftCard}
            effectiveManualCardProductId={effectiveManualCardProductId}
            finishLinkedCardSetup={finishLinkedCardSetup}
            finishManualCardAdd={finishDemoAdd}
            isUserBackedWallet={isUserBackedWallet}
            manualCardStatus={manualCardStatus}
            matchStatusByAccount={matchStatusByAccount}
            matchSuggestionByAccountId={matchSuggestionByAccountId}
            pendingLinkedAccounts={pendingLinkedAccounts}
            plaidAccounts={plaidAccounts}
            plaidError={plaidError}
            plaidStatus={plaidStatus}
            previewGradient={selectedCard.gradient}
            scanStep={scanStep}
            selectedManualCardProduct={selectedManualCardProduct}
            setDraftBusiness={setDraftBusiness}
            setDraftIssuer={setDraftIssuer}
            setDraftLast4={setDraftLast4}
            setDraftName={setDraftName}
            setManualCardProductId={setManualCardProductId}
            setPendingLinkedAccounts={setPendingLinkedAccounts}
            setScanStep={setScanStep}
            showConnectedAccounts={() => setScreen('connected-accounts')}
