      ...
      <div className="min-h-screen bg-black text-white">
        <div className="mx-auto flex min-h-screen w-full max-w-md flex-col px-4 pb-8 pt-6">
          ...
          {screen === 'connected-accounts' && user && (
            <section className="space-y-4" style={appleInfoFontStyle}>
              <div className="mb-1 flex items-center justify-between px-1">
                <button type="button" onClick={() => setScreen('profile')} className="rounded-full bg-[#2c2c2e] px-3 py-1.5 text-sm font-medium text-white/88">
                  Back
                </button>
              ...
              {plaidAccounts.length > 0 ? (
                <div className="space-y-3">
                  {plaidAccounts.map((account) => {
                    ...
                    const canViewCard = visibleCards.some((card) => card.id === cardId);
                              ...
                              {account.recentTransactions.slice(0, 3).map((transaction) => (
                                <div key={transaction.id} className="flex items-center justify-between gap-3 py-2">
                                  <div className="min-w-0">
                                    ...
                                    <p className="mt-0.5 text-[11px] text-white/44">{transaction.date} · {transaction.category}</p>
                                  </div>
                          ...
                          <div className="mt-4 flex items-center justify-between gap-3">
                            ...
                            <div className="flex shrink-0 items-center gap-2">
                              ...
                              <button
                                ...
                                onClick={() => {
                                  if (!canViewCard) return;
                                  selectCard(cardId);
                                  setScreen('wallet');
                                }}
                                className="rounded-full bg-white/10 px-3 py-1.5 text-xs font-medium text-white/82 disabled:cursor-not-allowed disabled:opacity-40"
                              >
                                View card
                              </button>
                            </div>
                ...
                <div className="rounded-[26px] border border-white/12 bg-[rgba(118,118,128,0.24)] px-4 py-4 shadow-[0_10px_24px_rgba(0,0,0,0.14),inset_0_1px_0_rgba(255,255,255,0.05)] backdrop-blur-2xl">
                  ...
                  <button type="button" onClick={openScanner} className="mt-4 w-full rounded-full bg-white px-4 py-3 text-sm font-medium text-[#060816]">
