...
## Active Gaps
- The first Use Now, Connected Accounts, post-Plaid match-card, shared match-suggestion component, and Plaid match suggestion hook boundaries are in place, but `WalletPrototype.tsx` still owns most wallet, Plaid, profile, and recommendation behavior.
...
## Next Best Actions
1. Move persisted Plaid account loading and transaction grouping into a tested local data hook now that the account-match suggestion map has its own boundary.
