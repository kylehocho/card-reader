...
## Active Gaps
- The first Use Now, Connected Accounts, post-Plaid match-card, shared match-suggestion component, Plaid match suggestion hook, persisted Plaid data hook, local transaction recommendation selector, and Plaid wallet action hook boundaries are in place, but `WalletPrototype.tsx` still owns wallet navigation, profile flows, merchant recommendation loading, and selected-card outcomes.
...
## Next Best Actions
1. Extract merchant recommendation loading and Use Now route-state effects into a focused hook so `/api/recommend-card` request states can be tested without the full wallet prototype.
